-- ── Agrega "Maquinaria pesada" a proyectos existentes ────────
insert into public.expense_categories (project_id, name, color, icon, type)
select id, 'Maquinaria pesada', '#f97316', 'construct', 'services'
from public.projects
where id not in (
  select project_id from public.expense_categories where name = 'Maquinaria pesada'
);
